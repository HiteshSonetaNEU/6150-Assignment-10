      ...
      {Object.keys(convertedData).map((dateKey) => {
        ...
        return (
            ...
            <Col xs={12} sm={6} md={6} lg={4} xl={3}>
                <Link to={`/${dateKey}`} state={convertedData[dateKey]} className='link'>
            <Card className="card">
              <Card.Body>
                <h2>{getWeatherIcon(convertedData[dateKey].data[0].weather[0].main)}</h2>
                <Card.Title>{`${dayName}, ${monthName} ${date.c.day}`}</Card.Title>
